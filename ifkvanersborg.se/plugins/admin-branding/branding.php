...
add_action( 'admin_menu', 'add_site_custom_settings_to_admin_menu' );
function add_site_custom_settings_to_admin_menu() {
	add_options_page( 'Generella Inställningar', 'Inställningar', 'edit_theme_options', 'custom-site-options', 'render_site_custom_settings_page' );
}


// Register setting-sections and setting-fields
add_action( 'admin_init', 'site_custom_settings_admin_init' );
function site_custom_settings_admin_init() {
    register_setting( 'site-custom-settings-group', 'site-settings', 'my_settings_sanitize' );
    add_settings_section( 'section-general', 'Generella inställningar', 'section_general_callback', 'site-custom-options' );
    add_settings_field( 'site-description', 'Slogan', 'site_description_callback', 'site-custom-options', 'section-general' );
}


// Callback for section one
function section_general_callback() {
    echo 'Ändra inställningar som berör hela hemsidan.';
}
	...
	'<div class="welcome-panel-content">
		...
		<div class="welcome-panel-column-container">
			...
			<div class="welcome-panel-column">
				<h4>Skapa nytt</h4>
				<ul>	
					<li><a href="'.site_url().'/wp-admin/post.php?post=2&action=edit" class="welcome-icon welcome-edit-page">Ändra startsidan</a></li>
					<li><a href="'.site_url().'/wp-admin/post.php?post=279&action=edit" class="welcome-icon welcome-add-page">Redigera personal</a></li>
			...
			<div class="welcome-panel-column welcome-panel-last">
				<h4>Se ändringar</h4>
				<ul>
					<li><div class="welcome-icon welcome-widgets-menus">Hantera <a href="'.site_url().'/wp-admin/widgets.php">widgets</a> eller ändra <a href="'.site_url().'/wp-admin/nav-menus.php">huvudmenyn</a></div></li>
					<li><a href="'.site_url().'/wp-admin/admin.php?page=custom-site-options" class="welcome-icon welcome-edit-page">Ändra slogan</a></li>
...
Kontakta mig på tel 0762-14 03 98, eller maila mig <a href="mailto:douglas.busck@gmail.com">här</a>.</p>';
}
...
function time_login_styling() { ?>
    <style type="text/css">
        body.login div#login h1 a {
            background-image: url(<?php echo get_bloginfo( 'template_directory' ) ?>/images/logotype_med.png);
            padding-bottom: 20px;
            height:105px;
            width:120px;
            margin:0 auto;
...
// Login logo url
add_filter( 'login_headerurl', 'time_login_logo_url' );
function time_login_logo_url() {
    return get_bloginfo( 'url' );
}

// Login logo url title
add_filter( 'login_headertitle', 'time_login_logo_url_title' );
function time_login_logo_url_title() {
    return 'Time Häggvall';
}
